...
bool startSession(JNIEnv* env, DeleteKeyEdge& edge) {
    ...
    if (!adapter) {
        LogTo("START: no adapter for version=%s loader=%s; refusing unsafe injection",
    ...
    if (!loader || !sakura::ValidateRuntimeAdapter(env, loader, *adapter)) {
        LogTo("START: selected adapter classes were not found; check loader/version selection");
...
BOOL APIENTRY DllMain(HMODULE, DWORD reason, LPVOID) { if (reason == DLL_PROCESS_ATTACH) { DisableThreadLibraryCalls(nullptr); InitializeProxy(nullptr); } return TRUE; }
