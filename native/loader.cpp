...
bool startSession(JNIEnv* env, DeleteKeyEdge& edge) {
    sakura::CompatibilityDecision compatibility = sakura::CheckCompatibility(g_runtimeProfile);
    if (!compatibility.allowed) {
        LogTo("START: refused unsafe profile version=%s loader=%s: %s",
              sakura::ToString(g_runtimeProfile.version),
              sakura::ToString(g_runtimeProfile.loader), compatibility.reason);
        return false;
    }

    const sakura::RuntimeAdapter* adapter = sakura::SelectRuntimeAdapter(g_runtimeProfile);
    if (!adapter) {
        LogTo("START: no adapter for selected profile");
        return false;
    }
    ...
    if (!valid) {
        LogTo("START: runtime binding validation failed; refusing injection");
        return false;
    }
